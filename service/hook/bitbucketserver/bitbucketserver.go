...
const BASE_URL = "bitbucket.lamudi.com";
...
func transformPullRequestEvent(pullRequest PullRequestEventModel) hookCommon.TransformResultModel {
	...
	if pullRequest.PullRequestInfo.SourceInfo.RepositoryInfo.FullName == pullRequest.PullRequestInfo.DestinationInfo.RepositoryInfo.FullName {
		...
		res, err := http.Head(fmt.Sprintf("https://%s/projects/%s/repos/%s", BASE_URL, PROJECT, pullRequest.PullRequestInfo.SourceInfo.RepositoryInfo.FullName))
		if err != nil {
			return hookCommon.TransformResultModel{
	...
	if pullRequest.PullRequestInfo.SourceInfo.RepositoryInfo.IsPrivate {
		// ssh://git@bitbucket.lamudi.com:7999/lmd/android-app.git
		sourceRepositoryURL = fmt.Sprintf("git@%s:%s/%s/%s.git", BASE_URL, PORT, PROJECT, pullRequest.PullRequestInfo.SourceInfo.RepositoryInfo.FullName)
	} else {
